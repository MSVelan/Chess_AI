...
import Chess.SmartMoveFinder as SmartMoveFinder
...
def loadImages():
    for i in ["b", "w"]:
        for j in ["R", "N", "B", "K", "Q", "p"]:
            IMAGES[i + j] = p.transform.scale(
                p.image.load("Chess/images/" + i + j + ".png"), (SQ_SIZE, SQ_SIZE)
            )
        ...
        for e in p.event.get():
            ...
            # mouse handler
                ...
                if not gameOver:
                    ...
                    if sqSelected == (row, col) or col >= len(
                        gs.board[0]
                    ):  # User clicks same square twice or user clicks the move log
                        sqSelected = ()  # deselect
...
def animateMove(move, screen, board, clock):
    ...
    for frame in range(frameCount + 1):
        r, c = (
            move.start_row + dR * frame / frameCount,
            move.start_col + dC * frame / frameCount,
        )
        drawBoard(screen)
        ...
        if move.pieceCaptured != "--":
            if move.isEnpassantMove:
                ...
                endSquare = p.Rect(
                    move.end_col * SQ_SIZE, enPassantRow * SQ_SIZE, SQ_SIZE, SQ_SIZE
                )
